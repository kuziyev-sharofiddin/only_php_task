...
class StoreRepository implements BaseInterface
{
    ...
    {
        $name = $data['name'];
        $stmt = $this->pdo->prepare("INSERT INTO stores (name) VALUES (:name)");
    ...
    public function update($id, array $data)
    {
        ...
        if (isset($id)) {
            if (!empty($data)) {
                ...
                $stmt = $this->pdo->prepare("UPDATE stores SET name = :name WHERE id = :id");
                $stmt->bindParam(':name', $name, \PDO::PARAM_STR);
                $stmt->bindParam(':id', $id, \PDO::PARAM_INT);
    ...
    {
        $stmt = $this->pdo->prepare("SELECT * FROM stores LIMIT $limit");
        $stmt->execute();
